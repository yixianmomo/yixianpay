...
class AppAlipay
{
    ...
    private $service = 'alipay.trade.app.pay';
    ...
    public function getPayPara()
    {
        ...
        foreach($para as $k=>$v){
         ...
         return json_encode($para); //返回原生支付需要用到的参数;
    ...
    protected function get_biz_content(){
        $biz_content = array(
           ...
           // 'body'=> $this->body,
            'subject'=> $this->subject,//'toalipay111',
    ...
    private function buildRequestPara($para_temp)
    {
        ...
        $para_sort['pay_success_url'] = 'http://m.sixishop.com/order/success_show/'.$this->out_trade_no.'/1';
        $para_sort['order_id'] = $this->out_trade_no;
        // echo $para_sort['sign']."<br/>";
        //原生支付签名结果不需要urlencode
        return $para_sort;
    }
