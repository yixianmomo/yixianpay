...
class WxPayApi
{
     ...
     * @return 成功时返回，其他抛异常 由微信支付demo修改而来，以适用于laravel框架
     */

    protected $wxconfig;

    public function __construct($wxconfig = array())
    {
    ...
    public function unifiedOrder($inputObj, $timeOut = 6)
    {
        ...
        if ($inputObj->GetTrade_type() == "NATIVE" && !$inputObj->IsProduct_idSet()) {
            die("统一支付接口中，缺少必填参数product_id！trade_type为NATIVE时，product_id为必填参数！");
        }

        //异步通知url未设置，则使用配置文件中的url
        if (!$inputObj->IsNotify_urlSet()) {
            die("统一支付接口中，缺少必填参数notify_url！");
        }

        $inputObj->SetAppid($this->wxconfig['app_id']);//公众账号ID
        $inputObj->SetMch_id($this->wxconfig['mch_id']);//商户号
        $inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);//终端ip
        //$inputObj->SetSpbill_create_ip("1.1.1.1");
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串

        //签名
        $inputObj->SetSign($this->wxconfig['pay_key']);
        $xml = $inputObj->ToXml();

        //$startTimeStamp = self::getMillisecond();//请求开始时间
        $response = self::postXmlCurl($xml, $url, false, $timeOut);
        $result = $inputObj->FromXml($response);
        //$WxPayResults = new WxPayResults();//为了节省时间不向腾讯服务器做上报数据和异常  以下方法相同
    ...
    public function orderQuery($inputObj, $timeOut = 6)
    {
        ...
        $inputObj->SetSign($this->wxconfig['pay_key']);//签名
        $xml = $inputObj->ToXml();
    ...
    public function refund($inputObj, $timeOut = 6)
    {
        ...
        $xml = $inputObj->ToXml();
        //$startTimeStamp = self::getMillisecond();//请求开始时间
        $response = self::postXmlCurl($xml, $url, true, $timeOut,$this->wxconfig['apiclient_cert'],$this->wxconfig['apiclient_key']);
        //$result = WxPayResults::Init($response);
        //self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
        $result = $inputObj->FromXml($response);
        return $result;
    ...
    public function refundQuery($inputObj, $timeOut = 6)
    {
        ...
        if (!$inputObj->IsOut_refund_noSet() &&
            ...
            die("退款查询接口中，out_refund_no、out_trade_no、transaction_id、refund_id四个参数必填一个！");
        }
        $inputObj->SetAppid($this->wxconfig['app_id']);//公众账号ID
        $inputObj->SetMch_id($this->wxconfig['mch_id']);//商户号
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
    ...
    public static function micropay($inputObj, $timeOut = 10)
    {
        ...
        if (!$inputObj->IsBodySet()) {
            die("提交被扫支付API接口中，缺少必填参数body！");
        } else if (!$inputObj->IsOut_trade_noSet()) {
            die("提交被扫支付API接口中，缺少必填参数out_trade_no！");
        } else if (!$inputObj->IsTotal_feeSet()) {
            die("提交被扫支付API接口中，缺少必填参数total_fee！");
        } else if (!$inputObj->IsAuth_codeSet()) {
            die("提交被扫支付API接口中，缺少必填参数auth_code！");
        }
    ...
    public static function reverse($inputObj, $timeOut = 6)
    {
        ...
        //检测必填参数
        if (!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
            die("撤销订单API接口中，参数out_trade_no和transaction_id必须填写一个！");
        }
    ...
    public static function report($inputObj, $timeOut = 1)
    {
        ...
        if (!$inputObj->IsInterface_urlSet()) {
            die("接口URL，缺少必填参数interface_url！");
        }
        if (!$inputObj->IsReturn_codeSet()) {
            die("返回状态码，缺少必填参数return_code！");
        }
        if (!$inputObj->IsResult_codeSet()) {
            //die("业务结果，缺少必填参数result_code！");
        }
        if (!$inputObj->IsUser_ipSet()) {
            //die("访问接口IP，缺少必填参数user_ip！");
        }
        if (!$inputObj->IsExecute_time_Set()) {
            //die("接口耗时，缺少必填参数execute_time_！");
        }
        $inputObj->SetAppid(WxPayConfig::APPID);//公众账号ID
        $inputObj->SetMch_id(WxPayConfig::MCHID);//商户号
    ...
    public static function shorturl($inputObj, $timeOut = 6)
    {
        ...
        if (!$inputObj->IsLong_urlSet()) {
            die("需要转换的URL，签名用原串，传输需URL encode！");
        }
        $inputObj->SetAppid(WxPayConfig::APPID);//公众账号ID
        $inputObj->SetMch_id(WxPayConfig::MCHID);//商户号
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
     ...
     * @throws WxPayException
     */
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30, $apiclient_cert_path = "", $apiclient_key_path = "")
    {
        $ch = curl_init();
        //设置超时
